...
### Polling / wait patterns (folded from weekly-retro 2026-05-27)
```bash
# Long workflow (>60s): NEVER wait on the webhook call — nginx 60s timeout makes
# n8nctl retry and you get DUPLICATE parallel executions. Fire-and-poll instead:
n8nctl workflow webhook 42 --no-wait
# → returns immediately; then poll the execution until terminal state:
EXEC_ID=$(n8nctl execution list --workflow 42 --limit 1 --jq '.[0].id')
until [ "$(n8nctl execution get "$EXEC_ID" --jq '.finished')" = "true" ]; do
  sleep 15
done
n8nctl execution get "$EXEC_ID" --jq '{status, stoppedAt}'

# Preferred when available (n8nctl >= 0.5): built-in wait with timeout
n8nctl workflow run 42 --trigger "Schedule Trigger" --wait --timeout 480000
```
