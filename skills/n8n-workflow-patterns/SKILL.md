...
description: Proven workflow architectural patterns + node cheat-sheet + ecommerce integration recipes for n8n. Use when building new workflows, designing workflow structure, choosing workflow patterns, planning architecture, picking the right node, or asking about webhook processing, HTTP API integration (auth/pagination/rate-limiting), database operations, AI agent workflows, scheduled tasks, ecommerce sync, or error-handling best practices.
---
...
Luôn start từ template (`<workflowRoot>/_templates/`, per `.n8nkit/config.json`):
...
## Node Cheat Sheet (merged from n8n-patterns)

**Triggers:** `Webhook` (external calls → Method/Path/Auth/Response Mode) · `Schedule` (cron/interval) ·
`n8n Trigger` (workflow events) · `Email Trigger (IMAP)`.
**Logic:** `IF` (binary) · `Switch` (multi-way) · `Merge` (Append/Keep-matches/Combine) · `Loop Over Items`
(rate-limited APIs) · `Wait` (pause/resume) · `Code` (JS, run-once vs per-item).
**Data:** `HTTP Request` (any REST) · `Set` (transform/rename) · `Aggregate` (items→1) · `Split Out` (1→many) ·
`Sort` · `Limit` · `Remove Duplicates` · `Date & Time`.
**Output:** `Respond to Webhook` · `Send Email` (SMTP) · `Slack/Discord/Telegram`.

## Ecommerce Integration Patterns (merged from n8n-patterns)

**Product Sync (A → B):** `Schedule(hourly) → HTTP(fetch A, paginated) → Code(transform A→B) →
Loop(batch 50, rate-limit) → HTTP(upsert B) → IF(errors) → Slack`.
**Order Webhook:** `Webhook(order.created) → Set(normalize) → HTTP(check inventory) → IF(in stock):
HTTP(confirm)+HTTP(fulfillment) ELSE HTTP(cancel/hold)+Email(ops)`.
**Price/Inventory Sync:** `Schedule(15min) → HTTP(fetch ERP) → Merge(match by SKU) → IF(qty/price changed)
→ Loop → HTTP(update platform)`.

## HTTP Request Tips (merged from n8n-patterns)

- **Auth:** Header Auth (Haravan/Shopify: `X-Shopify-Access-Token`) · Query-param signing (Shopee/Lazada/
  TikTok — generate signature in a Code node, pass signed URL) · OAuth2 (built-in credential type).
- **Pagination:** cursor-based (`Response Contains Next URL` → `next` field) · offset-based (Loop, increment
  offset by page_size) · page-number (Loop, increment page param).
- **Rate limiting:** `Loop Over Items` batch = API limit · add `Wait` (1-2s) between batches · check
  `X-RateLimit-Remaining`.
- **Node selection:** prefer native **HTTP Request 4.2** over community nodes (see `n8n-integrations`).
